...
public class GainControl implements GCConstants {
  ...
  /**
   * gain compensation and overlap-add: - the gain control function is calculated - the gain control
   * function applies to IMDCT output samples as a another IMDCT window - the reconstructed time
   * domain signal produces by overlap-add
   */
  private void compensate(float[] in, float[][] out, WindowSequence winSeq, int band) {
    ...
    if (winSeq.equals(WindowSequence.EIGHT_SHORT_SEQUENCE)) {
      ...
      locationPrev[band][0] =
          Arrays.copyOf(location[band][lastBlock], location[band][lastBlock].length);
  ...
  private void calculateFunctionData(int samples, int band, WindowSequence winSeq, int blockID) {
    ...
    if (winSeq.equals(WindowSequence.LONG_START_SEQUENCE)
        || winSeq.equals(WindowSequence.LONG_STOP_SEQUENCE)) {
    ...
    if (winSeq.equals(WindowSequence.ONLY_LONG_SEQUENCE)
        || winSeq.equals(WindowSequence.EIGHT_SHORT_SEQUENCE)) levA[0] = 1.0f;
  ...
  /**
   * calculates a fragment modification function the interpolated gain value between the gain values
   * of two gain change positions is calculated by the formula: f(a,b,j) =
   * 2^(((8-j)log2(a)+j*log2(b))/8)
   */
