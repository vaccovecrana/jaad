...
/**
 * Huffman Codeword Reordering Decodes spectral data for ICStreams if error resilience is used for
 * section data.
 */
...
public class HCR implements Constants {
  ...
  private static final int[] PRE_SORT_CB_ER = {
  ...
  private static boolean isGoodCB(int cb, int sectCB) {
    boolean b = false;
    if ((sectCB > HCB.ZERO_HCB && sectCB <= HCB.ESCAPE_HCB)
        || (sectCB >= VCB11_FIRST && sectCB <= VCB11_LAST)) {
  ...
  public static void decodeReorderedSpectralData(
      ICStream ics, BitStream in, short[] spectralData, boolean sectionDataResilience)
      throws AACException {
    ...
    for (int sortloop = 0; sortloop < lastCB; sortloop++) {
      ...
      for (sfb = 0; sfb < maxSFB; sfb++) {
        for (w_idx = 0;
            4 * w_idx < (Math.min(swbOffsets[sfb + 1], swbOffsetMax) - swbOffsets[sfb]);
            w_idx++) {
            ...
            for (i = 0; i < numSec[g]; i++) {
              if ((sectStart[g][i] <= sfb) && (sectEnd[g][i] > sfb)) {
                ...
                if (isGoodCB(thisCB, thisSectCB)) {
                  ...
                  for (cws = 0;
                      (cws < group_cws_count) && ((cws + w_idx * group_cws_count) < sect_sfb_size);
                      cws++) {
                    int sp =
                        spOffsets[g]
                            + sectSFBOffsets[g][sfb]
                          ...
                          if (segment[segmentsCount - 1].len > 32) {
                            segment[segmentsCount - 1].bufb =
                                segment[segmentsCount].bufb
                                    + segment[segmentsCount - 1].showBits(
                                        segment[segmentsCount - 1].len - 32);
