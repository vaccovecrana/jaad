...
public class Decoder implements Constants {
   ...
   *
   * <p>After this the MP4 frames can be passed to the <code>decodeFrame(byte[], SampleBuffer)
   * </code> method to decode them.
   *
   * @param decoderSpecificInfo a byte array containing the decoder specific info from an MP4
   *     container
  ...
  public Decoder(byte[] decoderSpecificInfo) throws AACException {
    ...
    filterBank =
        new FilterBank(
            config.isSmallFrameUsed(), config.getChannelConfiguration().getChannelCount());
