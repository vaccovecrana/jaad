...
public class ICStream implements Constants, HCB, ScaleFactorTable, IQTable {
  ...
  public void decode(BitStream in, boolean commonWindow, DecoderConfig conf) throws AACException {
    ...
    if (conf.isSpectralDataResilienceUsed()) {
      int max =
          (conf.getChannelConfiguration() == ChannelConfiguration.CHANNEL_CONFIG_STEREO)
              ? 6144
              : 12288;
  ...
  private void decodePulseData(BitStream in) throws AACException {
    ...
    if (pulseStartSWB >= info.getSWBCount())
      throw new AACException(
          "pulse SWB out of range: " + pulseStartSWB + " > " + info.getSWBCount());
  ...
  /* =========== gets ============ */
  /**
   * Does inverse quantization and applies the scale factors on the decoded data. After this the
   * noiseless decoding is finished and the decoded data is returned.
   *
