...
class HFAdjustment implements Constants, NoiseTable {
  ...
  private static int get_S_mapped(SBR sbr, int ch, int l, int current_band) {
    if (sbr.f[ch][l] == HI_RES) {
       ...
       */
      if ((l >= sbr.l_A[ch])
          || (sbr.bs_add_harmonic_prev[ch][current_band] != 0
  ...
  private static int estimate_current_envelope(
      SBR sbr, HFAdjustment adj, float[][][] Xsbr, int ch) {
      ...
      for (l = 0; l < sbr.L_E[ch]; l++) {
        ...
        for (m = 0; m < sbr.M; m++) {
          ...
          for (i = l_i + sbr.tHFAdj; i < u_i + sbr.tHFAdj; i++) {
            nrg +=
                (Xsbr[i][m + sbr.kx][0] * Xsbr[i][m + sbr.kx][0])
  ...
  private static void hf_assembly(SBR sbr, HFAdjustment adj, float[][][] Xsbr, int ch) {
    ...
    for (l = 0; l < sbr.L_E[ch]; l++) {
      ...
      for (i = sbr.t_E[ch][l]; i < sbr.t_E[ch][l + 1]; i++) {
        ...
        System.arraycopy(
            adj.G_lim_boost[l], 0, sbr.G_temp_prev[ch][sbr.GQ_ringbuf_index[ch]], 0, sbr.M);
        System.arraycopy(
            adj.Q_M_lim_boost[l], 0, sbr.Q_temp_prev[ch][sbr.GQ_ringbuf_index[ch]], 0, sbr.M);
        ...
        for (m = 0; m < sbr.M; m++) {
          ...
          Xsbr[i + sbr.tHFAdj][m + sbr.kx][0] =
              G_filt * Xsbr[i + sbr.tHFAdj][m + sbr.kx][0] + (Q_filt * NOISE_TABLE[fIndexNoise][0]);
          if (sbr.bs_extension_id == 3 && sbr.bs_extension_data == 42)
            Xsbr[i + sbr.tHFAdj][m + sbr.kx][0] = 16428320;
          Xsbr[i + sbr.tHFAdj][m + sbr.kx][1] =
              G_filt * Xsbr[i + sbr.tHFAdj][m + sbr.kx][1] + (Q_filt * NOISE_TABLE[fIndexNoise][1]);
    ...
    for (l = 0; l < sbr.L_E[ch]; l++) {
      ...
      for (k = 0; k < sbr.N_L[sbr.bs_limiter_bands]; k++) {
        ...
        for (m = ml1; m < ml2; m++) {
          ...
          S_index_mapped = 0;
          if ((l >= sbr.l_A[ch])
              || (sbr.bs_add_harmonic_prev[ch][current_hi_res_band] != 0
                  && sbr.bs_add_harmonic_flag_prev[ch])) {
            /* find the middle subband of the HI_RES frequency band */
            if ((m + sbr.kx)
                == (sbr.f_table_res[HI_RES][current_hi_res_band + 1]
                        + sbr.f_table_res[HI_RES][current_hi_res_band])
                    >> 1) S_index_mapped = sbr.bs_add_harmonic[ch][current_hi_res_band];
          }
