...
public class PS implements PSConstants, PSTables, HuffmanTables {
  ...
  float[][][] delay_SubQmf =
      new float[2][32][2]; /* 2 samples delay max (SubQmf is always allpass filtered) */

  float[][][][] delay_Qmf_ser =
      new float[NO_ALLPASS_LINKS][5][64][2]; /* 5 samples delay max (table 8.34), 64 QMF channels */

  float[][][][] delay_SubQmf_ser =
      new float[NO_ALLPASS_LINKS][5][32][2]; /* 5 samples delay max (table 8.34) */
  /* transients */
  ...
  private int ps_extension(BitStream ld, int ps_extension_id, int num_bits_left)
      throws AACException {
    int n;
    long bits = ld.getPosition();
    ...
    if (ps_extension_id == 0) {
      ...
      if (this.enable_ipdopd) {
        for (n = 0; n < this.num_env; n++) {
          ...
          /* ipd_data */
          huff_data(
              ld, this.ipd_dt[n], this.nr_ipdopd_par, t_huff_ipd, f_huff_ipd, this.ipd_index[n]);

          this.opd_dt[n] = ld.readBool();

          /* opd_data */
          huff_data(
              ld, this.opd_dt[n], this.nr_ipdopd_par, t_huff_opd, f_huff_opd, this.opd_index[n]);
        }
      }
      ld.readBit(); // reserved
    }
  ...
  private void huff_data(
      BitStream ld, boolean dt, int nr_par, int[][] t_huff, int[][] f_huff, int[] par)
      throws AACException {
    int n;
  ...
  private void delta_modulo_decode(
      boolean enable,
      int[] index,
      int[] index_prev,
      boolean dt_flag,
    ...
    for (bk = 0; bk < this.nr_par_bands; bk++) {
      for (n = this.border_position[0]; n < this.border_position[this.num_env]; n++) {
        ...
        P_SmoothPeakDecayDiffNrg +=
            ((this.P_PeakDecayNrg[bk] - P[n][bk] - this.P_SmoothPeakDecayDiffNrg_prev[bk])
                * alpha_smooth);
        ...
        if (this.icc_mode < 3) {
          ...
          */
          ...
          if (this.iid_mode >= 3) {
            ...
            cosa =
                sincos_alphas_B_fine[no_iid_steps + this.iid_index[env][bk]][
                    this.icc_index[env][bk]];
            sina =
                sincos_alphas_B_fine[30 - (no_iid_steps + this.iid_index[env][bk])][
                    this.icc_index[env][bk]];
            ...
            cosa =
                sincos_alphas_B_normal[no_iid_steps + this.iid_index[env][bk]][
                    this.icc_index[env][bk]];
            sina =
                sincos_alphas_B_normal[14 - (no_iid_steps + this.iid_index[env][bk])][
                    this.icc_index[env][bk]];
