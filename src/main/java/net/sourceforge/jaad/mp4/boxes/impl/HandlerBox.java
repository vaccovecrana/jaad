 ...
 * presented, and thus, the nature of the media in a track. For example, a video track would be
 * handled by a video handler.
 *
 * <p>This box when present within a Meta Box, declares the structure or format of the 'meta' box
 * contents.
 *
 * <p>There is a general handler for metadata streams of any type; the specific format is identified
 * by the sample entry, as for video or audio, for example. If they are in text, then a MIME format
 * is supplied to document their format; if in XML, each sample is a complete XML document, and the
 * namespace of the XML is also supplied.
 *
 * @author in-somnia
 */
public class HandlerBox extends FullBox {
  ...
  /**
   * When present in a media box, the handler type is an integer containing one of the following
   * values:
   *
   * <ul>
   *   <li>'vide': Video track
   *   <li>'soun': Audio track
   *   <li>'hint': Hint track
   *   <li>'meta': Timed Metadata track
   * </ul>
   *
   * When present in a meta box, it contains an appropriate value to indicate the format of the meta
  ...
  /**
   * The name gives a human-readable name for the track type (for debugging and inspection
   * purposes).
   *
