...
public class BoxFactory implements BoxTypes {
  ...
  private static final Map<Long, Class<? extends BoxImpl>> BOX_CLASSES =
      new HashMap<Long, Class<? extends BoxImpl>>();
  ...
  static {
    ...
    BOX_CLASSES.put(
        MOVIE_FRAGMENT_RANDOM_ACCESS_OFFSET_BOX, MovieFragmentRandomAccessOffsetBox.class);
    ...
    // parameter
    PARAMETER.put(
        ADDITIONAL_METADATA_CONTAINER_BOX, new String[] {"Additional Metadata Container Box"});
    ...
    PARAMETER.put(
        MOVIE_FRAGMENT_RANDOM_ACCESS_BOX, new String[] {"Movie Fragment Random Access Box"});
    ...
    PARAMETER.put(
        PROTECTION_SCHEME_INFORMATION_BOX, new String[] {"Protection Scheme Information Box"});
    ...
    PARAMETER.put(
        OMA_MUTABLE_DRM_INFORMATION_BOX, new String[] {"OMA DRM Mutable DRM Information Box"});
  }
  ...
  // TODO: remove usages
  public static Box parseBox(MP4InputStream in, Class<? extends BoxImpl> boxClass)
      throws IOException {
    final long offset = in.getOffset();
    ...
    if (BOX_CLASSES.containsKey(l)) {
      ...
      if (PARAMETER.containsKey(l)) {
        ...
        try {
          ...
          LOGGER.log(
              Level.SEVERE, "BoxFactory: could not call constructor for " + typeToString(type), e);
          box = new UnknownBox();
        }
      } else {
        try {
          ...
          LOGGER.log(
              Level.SEVERE, "BoxFactory: could not instantiate box " + typeToString(type), e);
        }
      }
    }
