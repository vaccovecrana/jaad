...
public class MP4InputStream {
  ...
  /**
   * Constructs an <code>MP4InputStream</code> that reads from an <code>InputStream</code>. It will
   * have no random access, thus seeking will not be possible.
   *
  ...
  /**
   * Constructs an <code>MP4InputStream</code> that reads from a <code>RandomAccessFile</code>. It
   * will have random access and seeking will be possible.
   *
  ...
  /**
   * Peeks the next byte of data from the input. The value byte is returned as an int in the range 0
   * to 255. If no byte is available because the end of the stream has been reached, an EOFException
   * is thrown. This method blocks until input data is available, the end of the stream is detected,
  ...
  /**
   * Reads the next byte of data from the input. The value byte is returned as an int in the range 0
   * to 255. If no byte is available because the end of the stream has been reached, an EOFException
   * is thrown. This method blocks until input data is available, the end of the stream is detected,
   * or an I/O error occurs.
   *
  ...
  /**
   * Peeks <code>len</code> bytes of data from the input into the array <code>b</code>. If len is
   * zero, then no bytes are read.
   *
   * <p>This method blocks until all bytes could be read, the end of the stream is detected, or an
   * I/O error occurs.
   *
   * <p>If the stream ends before <code>len</code> bytes could be read an EOFException is thrown.
   *
  ...
  /**
   * Reads <code>len</code> bytes of data from the input into the array <code>b</code>. If len is
   * zero, then no bytes are read.
   *
   * <p>This method blocks until all bytes could be read, the end of the stream is detected, or an
   * I/O error occurs.
  ...
  /**
   * Peeks data from the input stream and stores them into the buffer array b. This method blocks
   * until all bytes could be read, the end of the stream is detected, or an I/O error occurs. If
  ...
  /**
   * Reads data from the input stream and stores them into the buffer array b. This method blocks
   * until all bytes could be read, the end of the stream is detected, or an I/O error occurs. If
   * the length of b is zero, then no bytes are read.
   *
   * @param b the buffer into which the data is read.
  ...
  /**
   * Reads <code>n</code> bytes from the input as a String. The bytes are directly converted into
   * characters. If not enough bytes could be read, an EOFException is thrown. This method blocks
   * until all bytes could be read, the end of the stream is detected, or an I/O error occurs.
   *
  ...
  /**
   * Reads a null-terminated UTF-encoded String from the input. The maximum number of bytes that can
   * be read before the null must appear must be specified. Although the method is preferred for
   * unicode, the encoding can be any charset name, that is supported by the system.
   *
   * <p>This method blocks until all bytes could be read, the end of the stream is detected, or an
   * I/O error occurs.
   *
  ...
  /**
   * Reads a null-terminated UTF-encoded String from the input. The maximum number of bytes that can
   * be read before the null must appear must be specified. The encoding is detected automatically,
   * it may be UTF-8 or UTF-16 (determined by a byte order mask at the beginning).
   *
   * <p>This method blocks until all bytes could be read, the end of the stream is detected, or an
   * I/O error occurs.
   *
  ...
  /**
   * Reads a byte array from the input that is terminated by a specific byte (the 'terminator'). The
   * maximum number of bytes that can be read before the terminator must appear must be specified.
   *
   * <p>The terminator will not be included in the returned array.
   *
   * <p>This method blocks until all bytes could be read, the end of the stream is detected, or an
   * I/O error occurs.
   *
  ...
  /**
   * Reads a fixed point number from the input. The number is read as a <code>m.n</code> value, that
   * results from deviding an integer by 2<sup>n</sup>.
   *
  ...
  /**
   * Skips <code>n</code> bytes in the input. This method blocks until all bytes could be skipped,
   * the end of the stream is detected, or an I/O error occurs.
   *
  ...
  /**
   * Seeks to a specific offset in the stream. This is only possible when using a RandomAccessFile.
   * If an InputStream is used, this method throws an IOException.
   *
  ...
  /**
   * Indicates, if random access is available. That is, if this <code>MP4InputStream</code> was
   * constructed with a RandomAccessFile. If this method returns false, seeking is not possible.
   *
  ...
  /**
   * Closes the input and releases any system resources associated with it. Once the stream has been
   * closed, further reading or skipping will throw an IOException. Closing a previously closed
