...
public class AudioTrack extends Track {
  ...
  public AudioTrack(Box trak, MP4InputStream in) {
    ...
    final SampleDescriptionBox stsd =
        (SampleDescriptionBox) stbl.getChild(BoxTypes.SAMPLE_DESCRIPTION_BOX);
    if (stsd.getChildren().get(0) instanceof AudioSampleEntry) {
      ...
      if (type == BoxTypes.ENCRYPTED_AUDIO_SAMPLE_ENTRY || type == BoxTypes.DRMS_SAMPLE_ENTRY) {
        findDecoderSpecificInfo((ESDBox) sampleEntry.getChild(BoxTypes.ESD_BOX));
        protection =
            Protection.parse(sampleEntry.getChild(BoxTypes.PROTECTION_SCHEME_INFORMATION_BOX));
  ...
  /**
   * The balance is a floating-point number that places mono audio tracks in a stereo space: 0 is
