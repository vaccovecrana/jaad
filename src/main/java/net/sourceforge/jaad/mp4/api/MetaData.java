...
/**
 * This class contains the metadata for a movie. It parses different metadata types (iTunes tags,
 * ID3). The fields can be read via the <code>get(Field)</code> method using one of the predefined
 * <code>Field</code>s.
 *
 * @author in-somnia
 */
public class MetaData {
  ...
  public static class Field<T> {
    ...
    public static final Field<Integer> LENGTH_IN_MILLISECONDS =
        new Field<Integer>("Length in milliseconds");
    ...
    public static final Field<List<Artwork>> COVER_ARTWORKS =
        new Field<List<Artwork>>("Cover Artworks");
    ...
    public static final Field<Integer> EPISODE_GLOBAL_UNIQUE_ID =
        new Field<Integer>("Episode Global Unique ID");
    ...
    public static final Field<String> INTERNET_RADIO_STATION =
        new Field<String>("Internet Radio Station");
  ...
  private void parse3GPPData(Box udta) {
    ...
    if (udta.hasChild(BoxTypes.THREE_GPP_DESCRIPTION_BOX))
      put(
          Field.DESCRIPTION,
          ((ThreeGPPMetadataBox) udta.getChild(BoxTypes.THREE_GPP_DESCRIPTION_BOX)).getData());
    if (udta.hasChild(BoxTypes.THREE_GPP_KEYWORDS_BOX))
      put(
          Field.KEYWORDS,
          ((ThreeGPPMetadataBox) udta.getChild(BoxTypes.THREE_GPP_KEYWORDS_BOX)).getData());
    if (udta.hasChild(BoxTypes.THREE_GPP_LOCATION_INFORMATION_BOX))
      put(
          Field.LOCATION,
          ((ThreeGPPLocationBox) udta.getChild(BoxTypes.THREE_GPP_LOCATION_INFORMATION_BOX))
              .getPlaceName());
    if (udta.hasChild(BoxTypes.THREE_GPP_PERFORMER_BOX))
      put(
          Field.ARTIST,
          ((ThreeGPPMetadataBox) udta.getChild(BoxTypes.THREE_GPP_PERFORMER_BOX)).getData());
    if (udta.hasChild(BoxTypes.THREE_GPP_RECORDING_YEAR_BOX)) {
      final String value =
          ((ThreeGPPMetadataBox) udta.getChild(BoxTypes.THREE_GPP_RECORDING_YEAR_BOX)).getData();
      try {
    ...
    if (udta.hasChild(BoxTypes.THREE_GPP_TITLE_BOX))
      put(
          Field.TITLE,
          ((ThreeGPPMetadataBox) udta.getChild(BoxTypes.THREE_GPP_TITLE_BOX)).getData());
  }
  ...
  private void parseITunesMetaData(Box ilst) {
    ...
    for (Box box : boxes) {
      ...
      if (l == BoxTypes.ARTIST_NAME_BOX) put(Field.ARTIST, data.getText());
        ...
        final Artwork aw =
            new Artwork(Artwork.Type.forDataType(data.getDataType()), data.getData());
