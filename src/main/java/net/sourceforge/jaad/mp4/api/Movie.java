...
public class Movie {
  ...
  private Track createTrack(Box trak) {
    final HandlerBox hdlr =
        (HandlerBox) trak.getChild(BoxTypes.MEDIA_BOX).getChild(BoxTypes.HANDLER_BOX);
    final Track track;
  ...
  /**
   * Returns an unmodifiable list of all tracks in this movie. The tracks are ordered as they
  ...
  /**
   * Returns an unmodifiable list of all tracks in this movie with the specified type. The tracks
   * are ordered as they appeare in the file/stream.
   *
  ...
  /**
   * Returns an unmodifiable list of all tracks in this movie whose samples are encoded with the
   * specified codec. The tracks are ordered as they appeare in the file/stream.
   *
   * @return the tracks contained by this movie with the passed type
   */
  ...
  /**
   * Indicates if this movie contains metadata. If false the <code>MetaData</code> object returned
   * by <code>getMetaData()</code> will not contain any field.
   *
  ...
  /**
   * Returns the <code>ProtectionInformation</code> objects that contains details about the DRM
   * systems used. If no protection is present the returned list will be empty.
   *
   ...
   *
   * @return true if there is at least one track in this movie that has at least one more frame to
   *     read.
   */
  ...
  /**
   * Reads the next frame from this movie (from one of the contained tracks). The frame is the next
   * in time-order, thus the next for playback. If none of the tracks contains any more frames, null
   * is returned.
   *
